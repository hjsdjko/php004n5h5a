<template>
	<div class="main-content" :style='{"minHeight":"calc(100vh - 200px)","padding":"10px 0 30px","margin":"0 auto","color":"#333","background":"none","width":"calc(100% - 20px)","fontSize":"12px","height":"100%"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0px","margin":"0 0 10px","borderColor":"#ccc","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#eee","borderWidth":"1px","display":"flex","fontSize":"inherit","borderStyle":"dotted"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('nongyezhishi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0px 2px","fontSize":"14px","color":"#333","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('nongyezhishi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('nongyezhishi','农业分类')" type="success" @click="chartDialog1()">
			...
			<div :style='{"padding":"0px","borderColor":"#ccc","margin":"0 0 0px","borderRadius":"0px","background":"#eee","borderWidth":"1px 0px 1px 1px","width":"100%","borderStyle":"solid dotted dotted"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('nongyezhishi','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								浏览
							</el-button>
							<el-button class="edit" v-if=" isAuth('nongyezhishi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai19" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('nongyezhishi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('nongyezhishi','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="nongyefenleiChart1" style="width:100%;height:600px;"></div>
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var nongyefenleiChart1 = echarts.init(document.getElementById("nongyefenleiChart1"),'macarons');
        this.$http({
            url: "nongyezhishi/group/nongyefenlei",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
                let res = data.data;
                ...
                for(let i=0;i<res.length;i++){
                    xAxis.push(res[i].nongyefenlei);
                    yAxis.push(parseFloat((res[i].total)));
                    pArray.push({
                        value: parseFloat((res[i].total)),
                        name: res[i].nongyefenlei
                    })
                }
                var option = {};
                option = {
                ...
                // 使用刚指定的配置项和数据显示图表。
                nongyefenleiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    nongyefenleiChart1.resize();
                };
            }
        });
      })
	...
	this.$router.push({path:'/discussnongyezhishi',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	nongyezhishistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'nongyezhishi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
