		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"padding":"10px 0px","margin":"0 0 10px","borderColor":"#ccc","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"#eee","borderWidth":"1px","display":"flex","fontSize":"inherit","borderStyle":"dotted"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0 20px 0 0","margin":"0px 0 0px","color":"#333","alignItems":"center","textAlign":"left","display":"flex","justifyContent":"flex-start","borderRadius":"0px","flexWrap":"wrap","background":"none","width":"100%","fontSize":"inherit","order":"1"}'>
					<el-button class="add" v-if="isAuth('discussnongyezhishi','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia1" :style='{"margin":"0px 2px","fontSize":"14px","color":"#333","display":"inline-block"}'></span>
						新增
					</el-button>
					<el-button class="del" v-if="isAuth('discussnongyezhishi','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
			...
			<div :style='{"padding":"0px","borderColor":"#ccc","margin":"0 0 0px","borderRadius":"0px","background":"#eee","borderWidth":"1px 0px 1px 1px","width":"100%","borderStyle":"solid dotted dotted"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="edit" v-if=" isAuth('discussnongyezhishi','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
								<span class="icon iconfont icon-xiugai19" :style='{"margin":"0 0 0 4px","fontSize":"14px","color":"#333","height":"auto"}'></span>
								编辑
							</el-button>

							<el-button class="view" v-if="isAuth('discussnongyezhishi','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
...
import AddOrUpdate from "./add-or-update";
	...
	this.$router.push({path:'/discussdiscussnongyezhishi',query:{refid:id}});
    },
	backClick(){
		history.back()
	},
	...
	discussnongyezhishistatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'discussnongyezhishi/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
    ...
    deleteHandler(id ,refid ) {
      ...
      this.$confirm(`确定进行[${id ? "删除" : "批量删除"}]操作?`, "提示", {
        ...
        this.$http({
          url: "discussnongyezhishi/delete",
          method: "post",
          data: ids
        }).then(({ data }) => {
          if (data && data.code === 0) {
				this.$http({
					url: 'nongyezhishi/info/' + refid,
					method: "get",
				}).then(res=>{
