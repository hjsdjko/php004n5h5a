	...
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"10px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 20px)","fontSize":"12px","height":"100%"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-if="type!='info'"  label="评论内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.content" label="评论内容" prop="content">
				    <span :style='{"fontSize":"12px","lineHeight":"24px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.content"></span>
				</el-form-item>
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-if="type!='info'"  label="回复内容" prop="reply">
					<editor 
						style="min-width: 200px; max-width: 600px;"
				...
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.reply" label="回复内容" prop="reply">
...
export default {
    ...
    info(id) {
      this.$http({
        url: `discussnongyezhishi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `discussnongyezhishi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.discussnongyezhishiCrossAddOrUpdateFlag = false;
											this.parent.search();
