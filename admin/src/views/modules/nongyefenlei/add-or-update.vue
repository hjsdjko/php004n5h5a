<template>
	<div class="addEdit-block" :style='{"minHeight":"calc(100vh - 200px)","padding":"10px 0 30px","margin":"0 auto","color":"#666","background":"none","width":"calc(100% - 20px)","fontSize":"12px","height":"100%"}'>
		...
		>
			<template >
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="input" v-if="type!='info'"  label="农业分类" prop="nongyefenlei">
					<el-input v-model="ruleForm.nongyefenlei" placeholder="农业分类" clearable  :readonly="ro.nongyefenlei"></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' v-else class="input" label="农业分类" prop="nongyefenlei">
					<el-input v-model="ruleForm.nongyefenlei" placeholder="农业分类" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-if="type!='info' && !ro.image" label="图片" prop="image">
					<file-upload
						tip="点击上传图片"
						action="file/upload"
						:limit="3"
				...
				<el-form-item :style='{"padding":"0px 0","margin":"0 auto 8px","color":"inherit","borderRadius":"4px","background":"none","width":"100%","fontSize":"inherit"}' class="upload" v-else-if="ruleForm.image" label="图片" prop="image">
...
export default {
	data() {
		...
		return {
			...
			rules: {
				nongyefenlei: [
					{ required: true, message: '农业分类不能为空', trigger: 'blur' },
				],
				image: [
				],
			}
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `nongyefenlei/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.nongyefenleiCrossAddOrUpdateFlag = false;
											this.parent.search();
